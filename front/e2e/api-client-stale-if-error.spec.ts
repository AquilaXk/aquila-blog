...
test.describe("api client stale-if-error contract", () => {
  ...
  test("public detail and feed hooks retain stale meta beside React Query data", () => {
    ...
    expect(postQuerySource).toContain("const staleMetaRef = useRef<ApiFetchMeta | null>(null)")
    ...
    expect(postQuerySource).toContain("activeRouteIdRef.current === requestedRouteId")
    expect(postQuerySource).toContain("staleMeta: staleMetaRef.current")
