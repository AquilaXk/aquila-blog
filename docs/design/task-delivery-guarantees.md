...
- 저장 payload는 `schemaVersion`, `taskType`, `sensitivity`, 생성·만료 시각과 payload JSON을 가진 v2 envelope다. migration된 row는 v1 exact decoder만 사용하며 decode 실패 뒤 다른 version/class를 시도하지 않는다.
