...
- 표는 본문 내 context toolbar로 `행/열 추가`, `헤더 토글`, `셀 병합/분할`, `행/열/표 삭제`를 직접 지원하고, 열 폭 조절은 TipTap 기본 resize handle을 사용한다.
- 콜아웃/토글은 모두 compact 카드 편집기로 유지하고, 종류(콜아웃만)/제목/본문/미니 preview만 노출한다.
...
- mermaid block
- slash command
- desktop hover/selection block handle / mobile compact action bar / top-level block move
