...
require_pattern 'statuses:[[:space:]]*read' "frontend live verification must be allowed to read Vercel commit status"
require_pattern 'api\.github\.com/repos/\$\{GITHUB_REPOSITORY\}/commits/\$\{E2E_EXPECTED_FRONT_COMMIT_SHA\}/status' "frontend live verification must inspect commit status before live sha polling"
require_pattern 'Deployment rate limited' "frontend live verification must fail fast on Vercel rate limit"
require_pattern 'Vercel production deployment is rate limited' "frontend live verification must report actionable Vercel rate limit guidance"
...
require_pattern 'FRONT_BUILD_SHA_PATHS_PATTERN=.*contracts/' "frontend contract changes must force live build sha equality"
require_pattern 'FRONT_BUILD_SHA_PATHS_PATTERN=.*config/' "frontend config changes must force live build sha equality"
require_pattern 'FRONT_BUILD_SHA_PATHS_PATTERN=.*scripts/' "all frontend script changes must force live build sha equality"
...
require_pattern 'FRONT_LIVE_VERIFY_PATHS_PATTERN=.*\^\(front/' "frontend changes must run live frontend verification"
require_pattern 'FRONT_BUILD_SHA_PATHS_PATTERN=.*\^front/\(.*legal/' "Web legal policy source changes must force live frontend sha equality"
reject_pattern "FRONT_BUILD_SHA_PATHS_PATTERN='\\^front/'" "frontend e2e-only changes must not force live build sha equality"
reject_pattern 'FRONT_BUILD_SHA_PATHS_PATTERN=.*front/e2e/' "frontend e2e-only changes must not force live build sha equality"
