...
test.describe("admin posts workspace link contract", () => {
  ...
  test("관리자 글 목록은 V4 table row editor 진입 계약을 유지한다", () => {
    ...
    expect(listSource).toContain("onClick={isDeletedScope ? undefined : () => openEditorForRow(row)}")
    expect(listSource).toContain('role={isDeletedScope ? undefined : "button"}')
    ...
    expect(listStyleSource).toContain('tbody tr[role="button"]')
    ...
    expect(listSource).toContain("<TitleText>{buildRowTitle(row)}</TitleText>")
