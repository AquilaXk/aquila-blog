...
이번 출시 Soft-launch 범위는 **관리자 글 발행 + 비로그인 공개 열람 ONLY**다. 아래 기능은 enable하지 않으며, 켜려면 별도 issue + privacy/법무 evidence gate가 필요하다.

| 키 | Soft-launch 값 | 비고 |
| --- | --- | --- |
| `CUSTOM__MEMBER__SIGNUP__ENABLED` | `false` | 이메일 회원가입 |
...
Freeze 강제 경로: `deploy/env/env.contract.json` allowedValues + `.github/workflows/deploy.yml` `require_privacy_freeze_value`.

## Current Decision

| 항목 | 현재 판정 | 근거 | 다음 조치 |
| --- | --- | --- | --- |
| Soft-launch feature freeze | `pass` | #1127 Locked decision. signup/OAuth signup/RUM/AI SUMMARY는 deploy·env.contract에서 false/0 강제 | enable 요청은 별도 issue로만 처리한다. |
...
| #1127 | Closed | 필수 출시 전 완료 | Soft-launch 출시 범위와 privacy freeze 키 계약 고정 | Soft-launch=admin publish+anonymous public read ONLY; signup/OAuth signup/RUM/`CUSTOM__AI__SUMMARY__ENABLED` false/0; live smoke | 완료 |
