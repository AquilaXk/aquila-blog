...
require_pattern 'CURRENT_MAIN_SHA_INPUT:[[:space:]]*\$\{\{ github\.sha \}\}' "current main sha must be available for stale payload detection"
require_pattern 'stale workflow_run payload: deploy_sha=' "workflow_run payload freshness must be logged"
