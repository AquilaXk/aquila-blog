...
test.describe("api client stale-if-error contract", () => {
  ...
  test("public detail and feed hooks retain stale meta beside React Query data", () => {
    ...
    expect(postQuerySource).toContain("const [staleMetaByRouteId, setStaleMetaByRouteId]")
    ...
    expect(postQuerySource).toContain("[requestedRouteId]: result.meta")
    expect(postQuerySource).toContain("staleMeta: hasRouteId ? (staleMetaByRouteId[routeId] ?? null) : null")
