...
class CloudVideoUploadSessionCleanupScheduledJobTest {
    @Test
    @DisplayName("만료 세션 정리 작업은 설정된 batch size로 만료·stale 정리를 호출한다")
    fun `만료 세션 정리 작업은 설정된 batch size로 만료와 stale 정리를 호출한다`() {
        val service = mock(CloudVideoUploadSessionService::class.java)
        given(service.purgeExpiredSessions(50)).willReturn(1)
        given(service.purgeStaleIntermediateSessions(50)).willReturn(2)
        ...
        then(service).should().purgeStaleIntermediateSessions(50)
    }
}
