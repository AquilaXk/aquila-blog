...
require_pattern '\[ "\$\{GITHUB_EVENT_NAME\}" = "workflow_run" \] && \[ "\$\{DEPLOY_SHA\}" != "\$\{GITHUB_SHA\}" \]' "workflow_run must stop when the OIDC attestation source sha differs from the deploy sha"
require_pattern 'attestation-source-sha-mismatch' "workflow_run source-sha mismatch must be reported as a no-op"
require_pattern 'echo "backend_deploy=false"' "workflow_run source-sha mismatch must disable backend deployment"
require_pattern 'echo "front_deploy=false"' "workflow_run source-sha mismatch must disable front deployment"
require_pattern '## Deploy Boundary' "workflow_run source-sha mismatch must record a bounded deploy-boundary summary"
