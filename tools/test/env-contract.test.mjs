...
test("homeserver monitoring runtime files stay readable by container users", () => {
  ...
  assert.match(workflow, /chmod 0644 deploy\/homeserver\/monitoring\/prometheus\.yml/)
  assert.match(workflow, /chmod 0644 deploy\/homeserver\/monitoring\/alertmanager\.yml/)
  assert.match(workflow, /find deploy\/homeserver\/monitoring\/grafana\/dashboards .* -exec chmod 0644/)
  ...
  assert.match(deployScript, /find deploy\/homeserver\/monitoring -type d -exec chmod 0755/)
