...
4. Web에서 `yarn legal:check`를 실행하고, Platform에서는 pinned lock과 `ActiveLegalDocumentMetadata.kt`가 같은 terms/privacy version/hash를 가리키는지 확인한다.
...
- Platform lock checker와 backend acceptance metadata check.
...
- Web: `yarn legal:check`; Platform: `node tools/contracts/check-web-policy-lock.mjs`
