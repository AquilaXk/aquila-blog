...
class LegalAcceptanceApplicationService(
    ...
    override fun validateStoredSignupPolicyVersion(legalPolicyVersion: String?) {
        ...
        if (storedVersion != active.signupPolicyVersion) {
            throw AppException(ErrorCode.OAUTH_SIGNUP_POLICY, "약관 또는 개인정보처리방침이 변경되었습니다. 최신 내용을 확인하고 다시 동의해주세요.")
        }
    }

    override fun validateEmailSignupAcceptance(command: LegalAcceptanceCommand) {
        if (!command.age14OrOlder) {
            throw AppException(ErrorCode.MEMBER_BAD_REQUEST, "만 14세 이상인 경우에만 회원가입할 수 있습니다.")
        }
        if (!command.requiredPrivacyConfirmed) {
            throw AppException(ErrorCode.MEMBER_BAD_REQUEST, "개인정보 처리 필수 안내를 확인해야 회원가입할 수 있습니다.")
        }
        if (!command.overseasTransferAcknowledged) {
            throw AppException(ErrorCode.MEMBER_BAD_REQUEST, "국외 이전 및 외부 처리자 안내를 확인해야 회원가입할 수 있습니다.")
        }
